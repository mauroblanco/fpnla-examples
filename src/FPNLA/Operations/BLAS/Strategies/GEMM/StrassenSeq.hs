...
module FPNLA.Operations.BLAS.Strategies.GEMM.StrassenSeq () where
...
import           FPNLA.Operations.BLAS.Strategies.DataTypes (StrassenSeq,
                                                             getStrassenLimit)
...
instance (Elt e, GEMM s m v e) => GEMM (StrassenSeq s) m v e where
