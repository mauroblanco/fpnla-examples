...
module FPNLA.Operations.BLAS.Strategies.GEMM.MonadPar.BlocksPar () where
...
instance (NFData (m e), GEMM s m v e) => GEMM (BlocksPar_MP s) m v e where
    gemm (bctx, sctx) tmA tmB alpha beta mC = blasResultM . fromBlocks_m $ generatePar rA cB (\i j -> add_m (matMultIJ i j (bmC !! i !! j)) (map_m (beta *) (bmC !! i !! j)))
        where
            ...
            add_m = zipWith_m (+)
            generatePar m n f = runPar . parMap id $ generateBlocks m n f
